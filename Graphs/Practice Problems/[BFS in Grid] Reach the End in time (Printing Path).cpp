...
int count(vector<string>G, vector<vector<bool>>& vis,int i,int j,int n,int m){
	int dx[4] = {1,-1,0,0}; // DURL
	int dy[4] = {0,0,1,-1};

	map<pair<int,int>, pair<int,int>> par;  // for storing the parent of each cell
	vis[i][j] = true;
	...
	while(!q.empty()){
		...
		if(x==n-1 and y==m-1) {
			// printing shortest path also
			auto temp = make_pair(x,y);
			auto src = make_pair(i,j);
			while(temp!=src){
...
int32_t main(){
    ...
    cout<<"Shortest Path is of length: "<<ans<<endl;
